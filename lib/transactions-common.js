  ...
  if (!this._transaction_id) {
    ...
    if ((tx.forceCommitBeforeStart && !(options && (options.useExistingTransaction || options.forceCommitBeforeStart === false))) || (options && options.forceCommitBeforeStart)) {
      ...
      if (!(options && options.useExistingTransaction)) {
        throw new Meteor.Error('transaction-already-open', 'Transaction was already open', 'An attempt to start a transaction (\"' + description + '\") was made when a transaction was already open. Open transaction_id: ' + this._transaction_id);
      ...
      if (Meteor.isServer) {
        ...
        Meteor.call("_meteorTransactionsProcess", this._transaction_id, this._description, this._items, this._context, function (err, res) {
          if (err || !res) {
			...
			self._cleanReset(); // this is called in the runCallback function, which doesn't get run in the absence of a result
            self._callback(txid, callback, new Meteor.Error('commit-error', 'An error occurred, so transaction was rolled back.', JSON.stringify(err)), false, rethrowCommitError);
            return;
          }
          else {
...
Transact.prototype._callback = function (a, b, err, res, rethrowCommitError) {
  var c = (_.isFunction(a)) ? a : ((_.isFunction(b)) ? b : null);
  if (c) {
    c.call(this._lastTransactionData, err, res);
  }
  if (err && rethrowCommitError) {
    throw new Meteor.Error(err.error || 'transaction-commit-failed', err.reason || 'Reason unknown', err.details || JSON.stringify(err));
  }
}
...
Transact.prototype._openAutoTransaction = function (description, opt) {// console.log("Auto open check value for transaction_id: " + this._transaction_id + ' (Auto: ' + this._autoTransaction + ')');
  if (!this._transaction_id) {
    ...
    var options = {};
    if (opt) {
      _.each(["rethrowCommitError", "forceCommitBeforeStart", "useExistingTransaction"], function (o) {
        if (typeof opt[o] !== 'undefined') {
          options[o] = opt[o];  
        }
      });
    }
    this.start(description, options);
...
Transact.prototype._processTransaction = function (txid, description, items, context, cannotOverridePermissionCheck) {
  ...
  _.each(items, function (item, index) {
    if (item.action === "remove" && item.hardDelete && !item.instant) {
      ...
      // We overwrite the temporary version of the doc from removes on the client
      // Because chances are that the whole document was not available on the client
      // That said, hardDeleted docs (from the client) with {instant: true} are already
      // gone from the db collection, so we don't want to go looking for those (hence the ` && !item.instant` above)
      // This means is what causes data loss of incompletely published docs that are hard removed
      // using {instant: true} on the client
      // See the suggestions in README.md for avoiding this problem
  ...
  if (existingTransaction) {
    ...
    // Note: the transaction `state` field will already be set to 'pending'
    Transactions.update({_id: txid}, {$set: {items: items, context: context, description: description, lastModified: ServerTime.date()}});
  }
  else {
    var result = Transactions.insert({_id: txid, user_id: tx._userOrNull(), description: description, items: items, context: context, lastModified: ServerTime.date(), state: "pending"});
    if (!result) {
     throw new Meteor.Error('transaction-failed', 'Could not insert transaction document into database', 'Unable to commit transaction');
     return; 
    }
  }
  ...
  if (success) {
    ...
    tx.rollback.call(self, rollbackAllDoneItems);
  }
...
Meteor.methods({
  ...
  '_meteorTransactionsProcess' : function (txid, description, items, context) {
    ...
    var cannotOverridePermissionCheck = Meteor.isServer; // used to be just `true`, but this didn't allow for client side simulations to run properly
