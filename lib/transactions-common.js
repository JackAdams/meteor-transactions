...
Transactions = new Mongo.Collection(Meteor.settings.transactionsCollection || "transactions");
...
Transact.prototype.rollback = function (rollbackAllDoneItems) {
  ...
  if (tx.removeRolledBackTransactions) {
	...
	if (!Transactions.findOne({_id: this._transaction_id})) {	
	  Transactions.insert({_id: this._transaction_id, user_id: Meteor.userId(), description: this._description, items: items, context: this._context, lastModified: ServerTime.date(), state: "rolledBack"});
	}
	else {
...
Transact.prototype.insert = function (collection, newDoc, opt, callback) {
  ...
  if (this._permissionCheckOverridden(opt) || this._permissionCheck("insert", collection, newDoc, {})) {
    ...
    // var newId = self._doInsert(collection,_.extend(newDoc,{transaction_id:self._transaction_id}),opt,callback);
    self._pushToRecord("insert", collection, newId, {doc: _.extend(newDoc, {transaction_id: self._transaction_id})}, false, self._permissionCheckOverridden(opt));
...
Transact.prototype.remove = function (collection, doc, opt, callback) {
  ...
  var existingDoc = (!_.isObject(doc)) ? collection.findOne({_id: doc}) : doc; // , {transform: null}
...
Transact.prototype._unpackageForUpdate = function (data) {
  ...
  _.each(data, function (val) {
    var unpackagedValue;
    if (val.json) {
      unpackagedValue = JSON.parse(val.json);
    }
	else {
...
Meteor.methods({
  ...
  '_meteorTransactionsProcess' : function (txid, description, items, context) {
    ...
    // STEP 1 - Set initial state of transaction to "pending"
     if (!existingTransaction && !Transactions.insert({_id: txid, user_id: _.isFunction(Meteor.userId) && Meteor.userId() || null, description: description, items: items, context: context, lastModified: ServerTime.date(), state: "pending"})) {
		...
		try {
		  ...
		  switch (item.action) {
			...
			case 'update' :
			  ...
			  if (Collection.update({_id: item._id}, modifier), function (err, res) {
				if (err) {
				  success = false;
				  logErrors(err); 
				}
			...
			case 'remove' :
			  if (item.hardDelete) {
				...
				if (Collection.remove({_id: item._id}), function (err, res) {
				  if (err) {
					success = false;
				    logErrors(err);
				  }
				...
				if (Collection.update({_id: item._id}, {$set: _.extend(txData, {deleted: ServerTime.date()})}, function (err, res) {
				  if (err) {
					success = false;
				    logErrors(err);	
				  }
  ...
  '_meteorTransactionsUndo' : function (txid) {
    ...
    var expired = false;
    var queuedItems = [];
    var selector = (txid) ? { _id: txid} : {user_id: _.isFunction(Meteor.userId) && Meteor.userId() || null};
