...
(function() {
  ...
  Transact.prototype.insert = function(collection,newDoc,opt,callback) {
	...
	var options = (_.isObject(opt)) ? _.omit(opt,'tx') : null;
  ...
  Transact.prototype._closeAutoTransaction = function(opt,callback) {// console.log("Auto commit check value for autoTransaction: " + this._autoTransaction + ' (Auto: ' + this._autoTransaction + ')');
	if (this._autoTransaction) {
	  this.log("Auto committed: " + this._transaction_id); // + ' (Auto: ' + this._autoTransaction + ')';
	  this.commit(opt,opt && opt.callback || (_.isFunction(callback) && callback));	
	}
  }
