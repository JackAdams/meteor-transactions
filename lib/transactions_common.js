...
Transact.prototype.start = function(description) {
  ...
  if (!this._transaction_id) {
    ...
    this._transaction_id = Transactions.insert({user_id:Meteor.userId(),timestamp:(ServerTime.date()).getTime(),description:description});
...
Transact.prototype.remove = function(collection,doc,opt,callback) {
  ...
  function doRemove(collection,_id,sel,instant,opt,callback) {
    ...
    if (opt && ((typeof opt.softDelete !== 'undefined' && opt.softDelete) || (typeof opt.softDelete === 'undefined' && tx.softDelete))) {
      ...
      collection.update(sel,{$set:{deleted:(ServerTime.date()).getTime(),transaction_id:self._transaction_id}},callback);
      return;
    }
...
Transact.prototype.update = function(collection,doc,updates,opt,callback) {
  ...
  if (this._permissionCheckOverridden(opt) || this._permissionCheck("update", collection, existingDoc, updates)) {
    ...
    for (var i = 0; i < actionFieldsCount; i++) {
      ...
      if (opt && opt.instant) {
        try {
          doUpdate(collection,_id,updates,updateData,inverse,true,opt,callback,(i === (actionFieldsCount - 1)) ? true : false);
        ...
        (function(updateData,inverse,execute) {
          this._executionStack.push(function() {
            doUpdate(collection,_id,updates,updateData,inverse,false,opt,callback,execute);
  ...
  function doUpdate(collection,_id,updates,updateData,inverseData,instant,opt,callback,execute) {
	if (execute) {
	  if (!_.isFunction(callback)) {
		callback = undefined;
	  }
...
Transact.prototype._unpackageForUpdate = function(data) {
  ...
  _.each(data, function(val) {
    var unpackagedValue;
    if (val.value !== null && val.value.json) {
...
Meteor.methods({
  ...
  '_meteorTransactionsUndo' : function(txid) {
    ...
    if (lastTransaction && typeof lastTransaction.items !== 'undefined') {
	  ...
	  // Need to undo removes first, so that docs are available for undo updates if docs were updated before removal
      if (tx._checkTransactionFields(lastTransaction.items,lastTransaction._id)) {
        ...
        if (_.isArray(lastTransaction.items.updated)) {
          if (!expired) {
			 // Undo updates in reverse order in case a field has been updated by multiple actions
            _.each(lastTransaction.items.updated.reverse(), function(obj) { // console.log("Undoing update: ", JSON.stringify(obj));
        ...
        if (!expired) {
          ...
          Transactions.update({_id:lastTransaction._id},{$set:{undone:(ServerTime.date()).getTime()}});
        }
      }
      else {
    ...
    if (lastUndo && typeof lastUndo.items !== 'undefined') {
      ...
      if (tx._checkTransactionFields(lastUndo.items,lastUndo._id)) {
        if (_.isArray(lastUndo.items.removed)) {
          _.each(lastUndo.items.removed, function(obj) {
            if (obj.doc) {
              ...
              queuedItems.push(function(){tx.collectionIndex[obj.collection].update({_id:obj._id},{$set:{deleted:(ServerTime.date()).getTime(),transaction_id:lastUndo._id}})});
            }
          });
        }
        ...
        if (!expired) {
          ...
          Transactions.update({_id:lastUndo._id},{$unset:{undone:1}}); // ,$set:{timestamp:(ServerTime.date()).getTime()} -- LEADS TO UNEXPECTED RESULTS
        }
      }
      else {
