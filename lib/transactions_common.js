...
Transact.prototype.insert = function(collection,newDoc,opt,callback) {
  ...
  // But also need to weed out callback functions, as usual callbacks
  // don't make sense in the context of transactions
  var options = (_.isObject(opt)) ? _.omit(opt,'tx') : undefined;
  ...
  if (this._permissionCheckOverridden(opt) || this._permissionCheck("insert",collection,newDoc,{})) {
	...
	if ((typeof opt !== 'undefined' && opt.instant) || this._autoTransaction) {
	  try {
		var newId = collection.insert(_.extend(newDoc,{transaction_id:self._transaction_id}),options);
...
Transact.prototype.remove = function(collection,doc,opt,callback) {
  ...
  if (this._permissionCheckOverridden(opt) || this._permissionCheck("remove",collection,existingDoc,{})) {
	...
	if (opt && opt.instant) {
	  try {
		doRemove(collection,_id,sel,true,opt,options);
	  ...
	  this._executionStack.push(function() {
		doRemove(collection,_id,sel,false,opt,options);
  ...
  function doRemove(collection,_id,sel,instant,opt,options) {
	if (!_.isFunction(options)) {
	  options = undefined;  
	}
	if ((typeof opt.softDelete !== 'undefined' && opt.softDelete) || (typeof opt.softDelete === 'undefined' && tx.softDelete)) {
	  self._pushToRecord("removed",collection,_id,null,instant);
	  collection.update(sel,{$set:{deleted:(new Date).getTime(),transaction_id:self._transaction_id}},options);
	  return;
	}
...
Transact.prototype.update = function(collection,doc,updates,opt,callback) {
  ...
  if (this._permissionCheckOverridden(opt) || this._permissionCheck("update", collection, existingDoc, updates)) {
	...
	for (var i = 0; i < actionFieldsCount; i++) {
	  ...
	  if (opt && opt.instant) {
		try {
		  makeUpdate(collection,_id,updates,updateData,inverse,true,options);
		...
		this._executionStack.push(function() {
		  makeUpdate(collection,_id,updates,updateData,inverse,false,options);
  ...
  function makeUpdate(collection,_id,updates,updateData,inverseData,instant,options) {
	...
	collection.update({_id:_id},updates,options);
...
Transact.prototype._closeAutoTransaction = function(opt,callback,newId) {// console.log("Auto commit check value for autoTransaction: " + this._autoTransaction + ' (Auto: ' + this._autoTransaction + ')');
  if (this._autoTransaction) {
	...
	this.commit(opt,opt && opt.callback || (_.isFunction(callback) && callback),newId);	
  }
}
	  ...
	  if (_.isArray(lastTransaction.items.removed)) {
		_.each(lastTransaction.items.removed, function(obj) {
		  queuedItems.push(function(){
			if (obj.doc) {
			  // This is here because the original was removed
			  tx.collectionIndex[obj.collection].insert(obj.doc);
			}
			else {
