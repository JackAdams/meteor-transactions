...
(function() {
  ...
  var Transact = function() {
	...
	this.log = function(message) {if (this.logging) {console.log(message);}};
	...
	this.permissionCheck = function(command,collection,doc,modifier) { return true; }; // commands are "insert", "update", "remove"
	
	// For the purpose of filtering transactions later, a "context" field is added to each transaction
	// By default, we just throw in the document in question, merging in fields from any subsequent documents in the transaction on a last-write-wins basis
  ...
  Transact.prototype.commit = function(txid) {
	...
	if (_.isEmpty(this._items) && _.isEmpty(this._executionStack)) { // Have to do both checks in case of instant inserts that put nothing on this._executionStack but add to this._items
	  ...
	  this.log('Forbidden transaction removed: ' + this._transaction_id);
	  this.rollback();
	  return;
	}
	else {
  ...
  // Not intended for use in an app's actual code
  ...
  Transact.prototype.drillDown = function(obj,key) {
	...
	if (pieces.length > 1) {
	  ...
	  pieces.shift();
	  return this.drillDown(newObj,pieces.join('.'));
	}
	else {
	  if (obj) {
		return obj[key];
  ...
  // This is the object that gets exported for the app to interact with
