  ...
  Transactions.allow({
	...
	update: function(userId, doc, fields, modifier) { 
	  if (userId !== doc.user_id) {
		...
		if (tx._validateModifier(modifier)) {
		  return true;
		}
		else {
...
Transact.prototype._validateModifier = function (modifier) {
  var fields = modifier && modifier["$set"];
  if (!fields) {
   return null;
  }
  return this._checkTransactionFields(fields.items);
}

Transact.prototype._checkTransactionFields = function (items) {
...
Meteor.methods({
  ...
  '_meteorTransactionsUndo' : function(txid) {
	...
	var lastTransaction = Transactions.find(_.extend(selector, {$or:[{undone:null}, {undone:{$exists: false}}], expired: {$exists: false}}), {sort: {timestamp: -1}, limit:1}).fetch()[0];
	if (lastTransaction && typeof lastTransaction.items !== 'undefined' && (!txid || tx._checkTransactionFields(lastTransaction.items))) {
  ...
  '_meteorTransactionsRedo' : function(txid) {
	...
	var queuedItems = [];
	var selector = (txid) ? {_id:txid} : {user_id:Meteor.userId()};
	var lastUndo = Transactions.find(_.extend(selector,{undone:{$exists:true, $ne: null}, expired:{$exists:false}}), {sort: {undone: -1}, limit:1}).fetch()[0];
	if (lastUndo && typeof lastUndo.items !== 'undefined' && (!txid || tx._checkTransactionFields(lastUndo.items))) {
