...
(function() {
  ...
  Transact.prototype.start = function(description) {
	if (!this._transaction_id) {
	  ...
	  console.log('Started "' + description + '" with transaction_id: ' + this._transaction_id + ((this._autoTransaction) ? ' (auto started)' : ''));
  ...
  Transact.prototype.commit = function(txid) {
	...
	if (_.isEmpty(this._items) && _.isEmpty(this._executionStack)) { // Have to do both checks in case of instant inserts that put nothing on this._executionStack but add to this._items
	  ...
	  console.log('Forbidden transaction removed: ' + this._transaction_id);
	  this.rollback();
	  return;
	}
	else {
	...
	this._cleanReset(); console.log("Commit reset transaction manager to clean state");
  }
  ...
  Transact.prototype.rollback = function() {
	  ...
	  _.each(items.removed, function(obj) {
		if (obj.instant) {
		  try {
			...
			console.log('Rolled back remove');
		  }
		  catch (err) {
			console.log(err);
			error = true;
	  ...
	  _.each(items.updated, function(obj) {// console.log("Undoing update: ", obj);
		if (obj.instant && typeof obj.inverse !== 'undefined' && obj.inverse.command && obj.inverse.data) {
		  ...
		  try {  
			...
			console.log('Rolled back update');
		  }
		  catch (err) {
			console.log(err);
			error = true;
	  ...
	  _.each(items.inserted, function(obj) {
		if (obj.instant) {
		  ...
		  try {
			...
			console.log('Rolled back insert');
		  }
		  catch (err) {
			console.log(err);
			error = true;
	...
	self._cleanReset();
	console.log("Rollback reset transaction manager to clean state");
  }
  ...
  Transact.prototype.insert = function(collection,newDoc,opt) {
	...
	if (this._permissionCheckOverridden(opt) || this._permissionCheck("insert",collection,newDoc,{})) {
	  ...
	  this._rollback = true;
	  console.log("Insufficient permissions to insert this document into " + collection._name + ':', newDoc); // Permission to insert not granted
	  return;	
	}
  }
  ...
  Transact.prototype.remove = function(collection,doc,opt) {
	...
	if (this._permissionCheckOverridden(opt) || this._permissionCheck("remove",collection,existingDoc,{})) {
	  ...
	  this._rollback = true;
	  console.log("Insufficient permissions to remove this document from " + collection._name + ':', existingDoc); // Permission to remove not granted
	  return;
	}
  }
  ...
  Transact.prototype.update = function(collection,doc,updates,opt) {
	...
	if (this._permissionCheckOverridden(opt) || this._permissionCheck("update", collection, existingDoc, updates)) {
	  ...
	  this._rollback = true;
	  console.log("Insufficient permissions to update this document in " + collection._name + ':', existingDoc); // Permission to update not granted
	  return;
	}
  ...
  Transact.prototype._permissionCheck = function(action,collection,doc,updates) { // insert and remove send null for "updates" param, but this is where all the details of any update are found
	return this.permissionCheck(action,collection,doc,updates);
  }
